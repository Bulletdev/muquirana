module InsightsHelper
  ...
  # "Taxa de poupanca - Junho" / "Fluxo de caixa - Proximos 30 dias" -- a linha
  # de meta do card. Usa o periodo guardado no insight; cai no sujeito (nome de
  # conta ou lojista, vindo de facts) para insights sem periodo.
  ...
  # [valor, legenda] para a figura-chave alinhada a direita do card, a partir
  # dos facts de exibicao guardados na linha. Nil (sem figura) para linhas
  # escritas antes de facts serem persistidos -- elas se preenchem na proxima
  # execucao noturna.
  ...
  # A acao contextual de um card, montada a partir dos ids de sujeito que cada
  # gerador guarda em metadata. Retorna nil quando o sujeito nao resolve mais
  # (categoria/conta apagada) -- o card renderiza sem link. Consulta via
  # insight.family, nao Current, para que renders de broadcast funcionem.
  def insight_action(insight)
    ...
    when "net_worth_milestone"
      { text: t("insights.actions.net_worth_milestone"), href: dashboard_path }
    when "budget_at_risk", "budget_on_track"
  ...
  # O sentimento escolhe a cor; a prioridade so ordena o feed. Os dois sao
  # ortogonais -- uma grande melhora na taxa de poupanca e alta prioridade E boa
  # noticia, e nao pode renderizar vermelho. Vermelho fica reservado para um
  # saldo projetado negativo, seguindo a regra mais ampla do app de que nem
  # valores negativos sao vermelhos.
  ...
  # Derivado do tipo + a direcao ja guardada em metadata, entao linhas escritas
  # antes de uma mudanca no formato da metadata degradam para :warning, nunca
  # :negative.
  ...
  private
    # "Junho" para periodos alinhados ao mes, "Proximos 30 dias" / "Ultimos 30
    # dias" para janelas moveis, um intervalo explicito caso contrario; nome do
    # sujeito (conta, lojista) para insights sem periodo.
