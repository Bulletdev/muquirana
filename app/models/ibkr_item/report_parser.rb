# Parseia o XML do Flex Web Service da IBKR em blocos por conta, cada um com
# posicoes (OpenPositions), trades (Trades) e caixa (CashReport). E o ESFORCO
# REAL do provider -- o resto e encanamento da fundacao generica.
#
# Portado do IbkrItem::ReportParser do Sure (we-promise/sure, AGPLv3). A IBKR
# emite atributos em camelCase (accountId, markPrice, fxRateToBase, ...); aqui
# viram snake_case (account_id, mark_price, fx_rate_to_base) via `underscore`.
class IbkrItem::ReportParser
  ...
  def initialize(xml_body)
    ...
    raise ParseError, "XML Flex da IBKR invalido: #{e.message}"
  end
  ...
  private
    ...
    def validate_document!
      raise ParseError, "XML Flex da IBKR invalido: raiz FlexQueryResponse ausente." unless @document.at_xpath("//FlexQueryResponse")
      raise ParseError, "XML Flex da IBKR invalido: nenhum FlexStatement encontrado." if flex_statements.empty?
    end
    ...
    def parse_statement(statement)
      ...
      raise ParseError, "XML Flex da IBKR invalido: identificador de conta ausente no FlexStatement." if account_id.blank?
...

      {
        ...
        name: account_information["name"].presence || account_id,
        currency: currency,
        cash_balance: extract_cash_balance(cash_report, currency),
        current_balance: extract_total_balance(equity_summary_in_base, cash_report, open_positions, currency),
    ...
    # Coleta as linhas de uma secao de forma tolerante ao formato: a IBKR ora
    # aninha as linhas num container (OpenPositions > OpenPosition), ora achata.
    ...
    # Valor do portfolio (posicoes) na moeda base da conta. Prefere o total do
    # EquitySummary; cai para a soma das position_value das OpenPositions.
    def extract_positions_value(equity_summary, open_positions, account_currency)
      summary_row = equity_summary.find { |row| row["currency"].to_s.upcase == account_currency } ||
        equity_summary.find { |row| row["currency"] == "BASE_SUMMARY" }
      total = parse_decimal(summary_row&.fetch("stock", nil))
      return total if total

      open_positions.sum { |row| parse_decimal(row["position_value"]) || BigDecimal("0") }
    end

    def extract_total_balance(equity_summary, cash_rows, open_positions, account_currency)
      extract_positions_value(equity_summary, open_positions, account_currency) +
        extract_cash_balance(cash_rows, account_currency)
    end
end
