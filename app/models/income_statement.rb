class IncomeStatement
  ...
  def totals(transactions_scope: nil)
    transactions_scope ||= family.transactions.visible.where(accounts: { exclude_from_reports: false })
  ...
  private
    ...
    def build_period_total(classification:, period:)
      totals = totals_query(transactions_scope: family.transactions.visible.in_period(period).where(accounts: { exclude_from_reports: false })).select { |t| t.classification == classification }
