...
// Handles the transaction attachment upload widget: previews selected file
// names and enables/disables the submit button with basic client side hints.
// The server performs the definitive validation.
  ...
  updateSubmitButton() {
    ...
    if (hasFiles) {
      ...
      if (files.length > this.maxFilesValue) {
        isValid = false
        errorMessage = `Arquivos demais (max ${this.maxFilesValue})`
      }

      const oversizedFiles = files.filter(file => file.size > this.maxSizeValue)
      if (oversizedFiles.length > 0) {
        isValid = false
        errorMessage = `Arquivo grande demais (max ${Math.round(this.maxSizeValue / 1024 / 1024)}MB)`
      }
    } else {
    ...
    if (hasFiles && isValid) {
      ...
      this.submitButtonTarget.textContent = count === 1 ? "Enviar 1 arquivo" : `Enviar ${count} arquivos`
    } else if (errorMessage) {
      this.submitButtonTarget.textContent = errorMessage
    } else {
      this.submitButtonTarget.textContent = "Enviar"
    }
  }
}
